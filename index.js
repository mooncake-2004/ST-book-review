...
function dedupeExtensionSettings() { const entries = [...new Set(document.querySelectorAll('#stbr-extension-settings, .stbr-extension-settings'))]; entries.slice(1).forEach(entry => entry.remove()); if (!window.__stbrSettingsDeduper && document.body) { window.__stbrSettingsDeduper = new MutationObserver(() => dedupeExtensionSettings()); window.__stbrSettingsDeduper.observe(document.body, { childList: true, subtree: true }); } } function syncChatDraft() { if (!root || activeTab !== 'messages') return; state.drafts ||= {}; const input = root.querySelector('.stbr-room-input'); if (!input) return; const contact = state.contacts?.find(c => c.id === activeContactId) || state.contacts?.[0]; if (!contact) return; const key = roomKey(contact); if (!root.dataset.draftReady) { root.dataset.draftReady = '1'; root.addEventListener('click', event => { if (!event.target.closest('.stbr-send')) return; setTimeout(() => { const currentInput = root.querySelector('.stbr-room-input'); if (currentInput) currentInput.value = ''; const currentContact = state.contacts?.find(c => c.id === activeContactId) || state.contacts?.[0]; const currentKey = currentContact ? roomKey(currentContact) : key; state.drafts[currentKey] = ''; saveState(); }, 0); }); } input.value = state.drafts[key] || ''; input.addEventListener('input', () => { state.drafts[key] = input.value; clearTimeout(syncChatDraft.saveTimer); syncChatDraft.saveTimer = setTimeout(() => saveState(), 250); }); } function setupInlineReply() { if (!root || root.dataset.inlineReplyReady) return; root.dataset.inlineReplyReady = '1'; root.addEventListener('click', event => { const replyButton = event.target.closest('[data-action="reply"], .stbr-link-btn'); if (!replyButton || !/回复|回应/.test(replyButton.textContent || '')) return; setTimeout(() => { const composer = root.querySelector('.stbr-composer'); if (!composer) return; const replyKey = replyButton.dataset.id || replyButton.dataset.commentId || replyButton.dataset.targetId || ''; const currentButton = replyButton.isConnected ? replyButton : [...root.querySelectorAll('[data-action="reply"]')].find(button => !replyKey || button.dataset.id === replyKey || button.dataset.commentId === replyKey || button.dataset.targetId === replyKey) || root.querySelector('[data-action="reply"]'); if (!currentButton) return; const anchor = currentButton.closest('.stbr-reply-line, .stbr-comment-head, .stbr-message-actions') || currentButton; anchor.insertAdjacentElement('afterend', composer); composer.classList.add('stbr-inline-reply'); composer.scrollIntoView({ block: 'nearest', behavior: 'smooth' }); composer.querySelector('.stbr-room-input')?.focus(); }, 0); }); } function setupContactInteractions() { if (!root) return; root.querySelectorAll('.stbr-contact').forEach(contact => { contact.draggable = true; }); root.querySelectorAll('.stbr-chat-mode button').forEach((button, index) => { button.textContent = index === 1 ? '剧外' : '剧内'; }); if (root.dataset.contactInteractionReady) return; root.dataset.contactInteractionReady = '1'; let draggedId = ''; root.addEventListener('dragstart', event => { const contact = event.target.closest('.stbr-contact'); if (!contact) return; draggedId = contact.dataset.contactId || ''; contact.classList.add('is-dragging'); event.dataTransfer?.setData('text/plain', draggedId); event.dataTransfer.effectAllowed = 'move'; }); root.addEventListener('dragend', event => { event.target.closest('.stbr-contact')?.classList.remove('is-dragging'); draggedId = ''; }); root.addEventListener('dragover', event => { if (event.target.closest('.stbr-contact')) event.preventDefault(); }); root.addEventListener('drop', async event => { const target = event.target.closest('.stbr-contact'); if (!target) return; event.preventDefault(); const sourceId = draggedId || event.dataTransfer?.getData('text/plain'); const targetId = target.dataset.contactId; if (!sourceId || !targetId || sourceId === targetId) return; const from = state.contacts.findIndex(contact => contact.id === sourceId); const to = state.contacts.findIndex(contact => contact.id === targetId); if (from < 0 || to < 0) return; const [moved] = state.contacts.splice(from, 1); state.contacts.splice(to, 0, moved); await saveState(); render(); }); root.addEventListener('dblclick', async event => { const contact = event.target.closest('.stbr-contact'); if (!contact) return; const item = state.contacts.find(candidate => candidate.id === contact.dataset.contactId); if (!item) return; const name = prompt('修改联系人名称', item.name); if (name === null || !name.trim()) return; item.name = name.trim(); await saveState(); render(); }); } function setupReviewReplyInteractions() {
    ...
    root.addEventListener('click', event => {
        ...
        const comment = button.closest('.stbr-comment');
        const parentId = comment?.dataset.commentId || button.dataset.id;
        if (!comment || !parentId) return;

        root.querySelectorAll('.stbr-inline-review-reply').forEach(form => form.remove());

        const form = document.createElement('form');
        form.className = 'stbr-inline-review-reply';
        form.dataset.parentId = parentId;
        form.innerHTML = `<textarea class="stbr-input" name="reply" rows="2" placeholder="回复这条书评…" required></textarea><div><button type="submit" class="stbr-btn stbr-btn-primary">提交</button><button type="button" class="stbr-btn stbr-btn-soft" data-cancel-reply>取消</button></div>`;
    ...
    root.addEventListener('click', event => {
        event.target.closest('[data-cancel-reply]')?.closest('.stbr-inline-review-reply')?.remove();
    });

    root.addEventListener('submit', async event => {
        const form = event.target.closest('.stbr-inline-review-reply');
        if (!form) return;
        ...
        const comment = reviewFor(selectedMessage()).comments.find(item => item.id === parentId);
        if (!comment) return;
...
} function compactConversationHeader() {
...
async function init() {
    ...
    console.info('[ST Book Review] 1.7.6 loaded');
}
